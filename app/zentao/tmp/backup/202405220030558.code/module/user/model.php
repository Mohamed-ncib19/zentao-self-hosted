...
class userModel extends model
{
    ...
    public function createUser($account, $password)
    {
        ...
        if (strtolower($account) === 'guest') {
            throw new Exception('Guest account not allowed');
        }
        ...
        try {
            ...
            return ['message' => 'Error creating user: '. $e->getMessage()];
        }
    }
    ...
    {
        require_once __DIR__ . '/../../ldap/implement-ldap-auth/index.php';
        if(!$account or !$password) return ['status'=>false, 'message'=>'enter credentials'];
        $ldap_response = login_zentao($account,$password);

        if($ldap_response->status){
            
            $hashedPassword = md5(md5($password).$this->session->rand);
        /* Get the user first. If $password length is 32, don't add the password condition.  */
        $record = $this->dao->select('*')->from(TABLE_USER)
            ->where('account')->eq($account)
            ->beginIF(strlen($hashedPassword) < 32)->andWhere('password')->eq(md5($hashedPassword))->fi()
            ->andWhere('deleted')->eq(0)
            ->fetch();
        ...
        if($record)
        {
            $passwordLength = strlen($hashedPassword);
            if($passwordLength < 32)
            ...
            {
                $hash = $this->session->rand ? md5($record->password . $this->session->rand) : $record->password;
                $user = $hashedPassword == $hash ? $record : '';
            }
            elseif($passwordLength == 40)
            {
                $hash = sha1($record->account . $record->password . $record->last);
            ...
            if(!$user and md5($hashedPassword) == $record->password) $user = $record;
        }else{
            
            return ['status' => false , 'message' => 'user do not exist in zentao database'];
        }
        if($user)
        {
        ...
        return ['status' => false , 'message' => $ldap_response->message];
    }
    }
