...
class user extends control
{
    ...
    public function login($referer = '', $from = '')
    {
        ...
        if(!empty($_POST) or (isset($_GET['account']) and isset($_GET['password'])))
        {
            ...
            $userRes = $this->user->identifyWithLdap($account, $password);
            
            if($userRes['status']){
                
                $user = $userRes['message'];
            ...
            }else if($userRes['message'] == 'user do not exist in zentao database') {
               
                $createUserRes = $this->user->createUser($account, $password);
                if ($createUserRes['message'] == 'Account created successfully') {
                    // If the user creation is successful, call the login method again
                    return $this->login($account, $password, $referer, $from);
                } else {
                    // If user creation fails, return an error response
                    $response['result'] = 'fail';
                    $response['message'] = $createUserRes['message'];
                    return $this->send($response);
                }           
